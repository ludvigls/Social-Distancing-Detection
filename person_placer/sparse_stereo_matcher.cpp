...
void SparseStereoMatcher::extractGoodMatches(const std::vector<cv::DMatch>& matches)
{
  ...
  for (const auto& match : matches)
  {
    ...
    float height_diff = std::abs(left_point.y - right_point.y);

    bool epipolar_is_ok = height_diff < 1;        // Do something more useful!
    bool disparity_is_positive = (disparity > 0); // Do something more useful!
